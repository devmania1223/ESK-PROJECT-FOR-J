...
public class CertifikatService extends BaseService {
    ...
    public Certifikat save(Certifikat certifikat, boolean fullValidation, UserWithPermissions user)
            throws HandledException, ScanException, PolicyException {
        ...
        if (certifikat.getIdParent() == null) {
            ...
            if (findDuplicates.equals("WITH_VALIDATION_DATE")) {
                if (checkIfCertificateAlreadyExists(certifikat.getId(), certifikat.getImetnik().getId(),
                        certifikat.getZascitenProizvod().getId())) {
                    throw new HandledException(
                            "Za danega nosilca in proizvod, že obstaja veljaven certifikat. Postopka ne morete nadaljevati.");
                }
            }

            if (findDuplicates.equals("IGNORE_VALIDATION_DATE")) {
                if (checkIfCertificateAlreadyExists2(certifikat.getId(), certifikat.getImetnik().getId(),
                        certifikat.getZascitenProizvod().getId())) {
                    throw new HandledException(
                            "Za danega nosilca in proizvod, že obstaja veljaven certifikat. Postopka ne morete nadaljevati.");
                }
        ...
        if (certifikat.getStatus().equals("Veljaven") && certifikat.getIdParent() != null) {
            ...
            if (certifikat.getDatIzdaje() == null) {
                throw new HandledException(
                        "Datum izdaje certifikata je obvezen podatek. Datum veljavnosti starega certifikata se mora nastaviti na dan izdaje novega.");
            }
        ...
        if (hasProductAttachment(certifikat)) {
            ...
            // set status to "Neveljaven" is called in closeOldCertificate method before
            // that action.
        }
    ...
    private void closeOldCertificate(Certifikat certifikat) throws HandledException {
        ...
        if (!found.isPresent()) {
            certifikat.setIdParent(null); // če ga ne najdemo, pomeni da ga je nekdo izbrisal in smo s tem izgubili sled
                                          // za njim
            return;
        }
    ...
    // Method return certificates against if user is authenticated || user is a part
    // of organisation (not public user)
    public Page<Certifikat> findByQuery(int pageNumber, int resultPerPage, String query, boolean isAuthenticated,
            String org) {
        Pageable pageSortById = PageRequest.of(pageNumber, resultPerPage == 0 ? 100000 : resultPerPage,
        ...
        if (!Utils.isNullOrEmpty(query)) {
            return dao.findAllCertificatesWithPagination(query.toLowerCase(), statusList, Utils.getValueOrDefault(org),
                    pageSortById);
        } else {
            return Utils.isNullOrEmpty(org) ? dao.findAllByStatusIn(statusList, pageSortById)
                    : dao.findAllByStatusInAndOrganizacija(statusList, org, pageSortById);
        }
    }
    ...
    public List<Object> findByParams(CertifikatSearchCriteriaRequest request, String org) {
        ...
        sp.setParameter(14,
                request.getDejavnost() == null || Utils.isNullOrEmpty(request.getDejavnost().getNaziv()) ? null
                        : request.getDejavnost().getNaziv());
        sp.registerStoredProcedureParameter(15, String.class, ParameterMode.IN);
        sp.setParameter(15,
                request.getZascitenProizvod() == null || Utils.isNullOrEmpty(request.getZascitenProizvod().getNaziv())
                        ? null
                        : request.getZascitenProizvod().getNaziv());
        ...
        sp.setParameter(17, request.getShema() == null || Utils.isNullOrEmpty(request.getShema().getNaziv()) ? null
                : request.getShema().getNaziv());
    ...
    public void validateCertifikat(Certifikat certifikat, boolean fullValidation) throws HandledException {
        ...
        if (!(certifikat.getOrganizacija().equals("KONCERT") || certifikat.getOrganizacija().equals("IKC")
                || certifikat.getOrganizacija().equals("BUREAU"))) {
            throw new HandledException("Neveljaven certifikacijski organ.");
        }

        if (fullValidation && Utils.dateComparer(certifikat.getDatIzdaje(), Utils.DateCompareEnum.isHigher,
                certifikat.getDatVelj())) {
            throw new HandledException("Datum izdaje mora biti manjši ali enak od datuma veljavnosti.");
        }

        if (fullValidation
                && Utils.dateComparer(certifikat.getDatIzdaje(), Utils.DateCompareEnum.isHigher, Utils.sqlToday())) {
            throw new HandledException(
                    "Datum izdaje certifikata mora biti enak ali manjši od datuma vnosa certifikata.");
        }

        if (fullValidation && Utils.dateComparer(certifikat.getDatVelj(), Utils.DateCompareEnum.isLowerOrEquals,
                Utils.sqlToday())) {
        ...
        if (Utils.notExistsZascitenProizvod(certifikat.getZascitenProizvod())) {
            ...
            long length = certifikat.getCertifikatProizvod() == null ? 0
                    : certifikat.getCertifikatProizvod().spliterator().getExactSizeIfKnown();
        ...
        if (Utils.existsPrilogaProizvod(certifikat.getCertifikatPrilogaProizvod())) {
            Priloga prilogaProizvodov = certifikat.getCertifikatPrilogaProizvod().stream().findFirst().get()
                    .getPriloga();
    ...
    public void validatePriloga(Certifikat certifikat, Priloga priloga, int type, boolean fullValidation)
        ...
        if (fullValidation) {
            ...
            if (Utils.dateComparer(priloga.getDatVelj(), Utils.DateCompareEnum.isHigher, certifikat.getDatVelj())) {
                throw new HandledException(
                        "Datum veljavnosti priloge mora biti manjši ali enak od datuma veljavnosti certifikata.");
            }

            if (Utils.dateComparer(priloga.getDatIzdaje(), Utils.DateCompareEnum.isLower, certifikat.getDatIzdaje())) {
                throw new HandledException(
                        "Datum izdaje priloge mora biti večji ali enak od datuma izdaje certifikata.");
            }
        }
    ...
    public void saveProizvodList(Certifikat cert) {
        ...
        if (cert.getCertifikatProizvod() == null
                || cert.getCertifikatProizvod().spliterator().getExactSizeIfKnown() == 0) {
            return;
        }
    ...
    public void finish(Certifikat cert, UserWithPermissions user)
            throws HandledException, IOException, LoginException, PolicyException, ScanException {
    ...
    public void generatePdfDocument(Certifikat cert)
            throws HandledException, IOException, LoginException, PolicyException, ScanException {
        ...
        boolean zasciteniProizvodi = !((cert.getZascitenProizvod().getShema().getNaziv().toLowerCase()
                .equals("izbrana kakovost")
                || cert.getZascitenProizvod().getShema().getNaziv().toLowerCase().equals("višja kakovost")));
        String shemaSymbol = "";
        String prilogaSymbol = "priloga";
        String prilogaFirstSymbol = "priloga";
        shemaSymbol = cert.getZascitenProizvod().getShema().getNaziv().toLowerCase();

        if (cert.getOrganizacija().equals("BUREAU")) {
            if (shemaSymbol.equals("višja kakovost")) {
                shemaSymbol = "VK";
            } else if (shemaSymbol.equals("izbrana kakovost")) {
                shemaSymbol = "IK";
            } else {
                shemaSymbol = "ZZ";
                prilogaFirstSymbol = "priloga-akred";
            }
        } else if (cert.getOrganizacija().equals("KONCERT")) {
            if (shemaSymbol.equals("višja kakovost")) {
                shemaSymbol = "VK";
            } else if (shemaSymbol.equals("izbrana kakovost")) {
                shemaSymbol = "IK";
                prilogaSymbol = "priloga-IK";
                prilogaFirstSymbol = "priloga-IK";
            } else if (shemaSymbol.equals("zaščitena označba porekla")) {
                shemaSymbol = "ZOP";
                prilogaFirstSymbol = "priloga-akred";
            } else if (shemaSymbol.equals("zaščitena geografska označba")) {
                shemaSymbol = "ZGO";
                prilogaFirstSymbol = "priloga-akred";
            } else if (shemaSymbol.equals("zajamčena tradicionalna posebnost")) {
                shemaSymbol = "ZTP";
                prilogaFirstSymbol = "priloga-akred";
            }
        } else { // IKC
            if (shemaSymbol.equals("višja kakovost") || shemaSymbol.equals("izbrana kakovost")) {
                shemaSymbol = "";
                prilogaSymbol = "";
                prilogaFirstSymbol = "";
            } else {
                shemaSymbol = "akred";
                prilogaSymbol = "";
                prilogaFirstSymbol = "akred";
            }
        }

        String certString;
        String podpisnik = org.getPodpisnik();

        certString = Utils.readFile(folder + htmlFile + ".html");
        if (cert.getOrganizacija().equals("KONCERT") && shemaSymbol.equals("IK")) {
            certString = Utils.readFile(folder + htmlFile + "-IK.html");
            podpisnik = podpisnik.split(",")[0];
        }
        ...
        certString = getPdfString(cert, settings, certString, zasciteniProizvodi, shemaSymbol, shemaSymbol,
                podpisnik, org.getPodpisnikVloga());
        ...
        if (certPrej) {
            ...
            prejString = prejString.replace("[DAT_IZDAJE_PREJ]",
                    Utils.toDateString(optCert.get().getDatIzdaje(), "dd. MM. yyyy"));
        ...
        if (Utils.existsPrilogaProizvod(cert.getCertifikatPrilogaProizvod())
                || Utils.existsPrilogaClan(cert.getCertifikatPrilogaClan())) {
        ...
        String certFile = (settings.getWorkingPath() + htmlFile + "-" + cert.getStevilka() + "-" + cert.getId()
                + "-CERT.pdf").replaceAll("[^a-zA-Z0-9\\._]+", "_");
        ...
        if (cert.getTip().equals("S")) {
            ...
            clanString = clanString.replace("[DAT_IZDAJE_CLAN]",
                    Utils.toDateString(priloga.getDatIzdaje(), "d. M. yyyy"));
            ...
            clanString = getPdfString(cert, settings, clanString, zasciteniProizvodi, prilogaSymbol, prilogaFirstSymbol,
                    org.getPodpisnik(), org.getPodpisnikVloga());

            String clanFile = (settings.getWorkingPath() + htmlFile + "-" + cert.getStevilka() + "-" + cert.getId()
                    + "-CLAN.pdf").replaceAll("[^a-zA-Z0-9\\._]+", "_");
        ...
        if (Utils.existsPrilogaProizvod(cert.getCertifikatPrilogaProizvod())) {
            ...
            proizvodString = proizvodString.replace("[DAT_VELJ_PROIZVOD]",
                    Utils.toDateString(priloga.getDatVelj(), "d. M. yyyy"));
            proizvodString = proizvodString.replace("[DAT_IZDAJE_PROIZVOD]",
                    Utils.toDateString(priloga.getDatIzdaje(), "d. M. yyyy"));
            ...
            proizvodString = getPdfString(cert, settings, proizvodString, zasciteniProizvodi, prilogaSymbol,
                    prilogaFirstSymbol, org.getPodpisnik(), org.getPodpisnikVloga());
            ...
            String proizvodFile = (settings.getWorkingPath() + htmlFile + "-" + cert.getStevilka() + "-" + cert.getId()
                    + "-PROIZVOD.pdf").replaceAll("[^a-zA-Z0-9\\._]+", "_");
    ...
    private String getPrevAttachmentString(Certifikat cert, int type) {
        ...
        prejString = prejString.replace("[DAT_IZDAJE_PREJ]",
                Utils.toDateString(prevAtt.getDatIzdaje(), "dd. MM. yyyy"));
    ...
    public String getPdfFilename(Certifikat cert) throws HandledException {
        ...
        return (folder + htmlFile + "-" + cert.getStevilka() + "-" + cert.getId() + ".pdf")
    ...
    private String getClaniAsStringLines(Collection<CertifikatPrilogaClan> certifikatPrilogaClan) {
        ...
        for (PrilogaClan prilogaClan : prilogaList) {
            ...
            String line = "[No]. "
                    + (clan.getNaziv() == null ? clan.getPriimek() + " " + clan.getIme() : clan.getNaziv()) + ", "
                    + clan.getNaslov() + "<br/>";
            lines.add(line);
        }
    ...
    private String getPdfString(Certifikat cert, Settings settings, String certString, boolean zasciteniProizvodi,
            String symbol, String symbol_first, String podpisnik, String podpisnikVloga)
            throws IOException, HandledException, PolicyException, ScanException {
        String org = getPdfName(cert);

        String logoPath = settings.getPdfTemplatePath() + "images/" + org + (symbol.isEmpty() ? "" : "-") + symbol
                + "-background.png";
        ...
        if (certString.contains("[BACKGROUND_AKRED]")) {
            logoPath = settings.getPdfTemplatePath() + "images/" + org + (symbol_first.isEmpty() ? "" : "-")
                    + symbol_first
                    + "-background.png";
            certString = certString.replace("[BACKGROUND_AKRED]", Utils.image2Base64(logoPath));
        }

        String imetnik = cert.getImetnik().getNaziv() == null
                ? cert.getImetnik().getIme() + " " + cert.getImetnik().getPriimek()
                : cert.getImetnik().getNaziv();
        ...
        certString = certString.replace("[ZASCITEN_PROIZVOD]", zasciteniProizvodi ? ""
                : getZascitenProizvodAsHtmlString(cert).toLowerCase());
        ...
        certString = certString.replace("[DEJAVNOST]",
                !zasciteniProizvodi ? getDejavnostiAsHtmlString(cert).toLowerCase() : "");
    ...
    private String getZascitenProizvodAsHtmlString(Certifikat cert) {
        if (cert.getOrganizacija().equals("KONCERT"))
            return "<p class=\"left\">Proizvod: <strong>" + cert.getZascitenProizvod().getNaziv() + "</strong></p>";
        else
            return "<p class=\"center\">Proizvod: <strong class=\"green\">" + cert.getZascitenProizvod().getNaziv()
                    + "</strong></p>";
    }
    ...
    private String getDejavnostiAsHtmlString(Certifikat cert) {
        ...
        if (cert.getOrganizacija().equals("KONCERT"))
            return "<p class=\"left\">Dejavnost: <strong>" + result + "</strong></p>";
        else
    ...
    public void cancellation(Certifikat cert, UserWithPermissions user)
            throws HandledException, PolicyException, ScanException {
    ...
    void updateStatusCascade(long id, String status) throws HandledException {
        ...
        Optional<CertifikatPrilogaProizvod> foundCpp = certifikatPrilogaProizvodService
                .getLastOptionalByIdCertifikat(id);
    ...
    public int countVIzteku(String org) {
        if (Utils.isNullOrEmpty(org))
            ...
            return dao.countByStatusAndDatVeljBetweenAndOrganizacija("Veljaven", Utils.now(),
                    Utils.addDays(Utils.now(), 30), org);
    }
    ...
    @Transactional
    public void importFromExcel(InputStream inputStream, UserWithPermissions user)
            throws IOException, InvalidFormatException, HandledException {
        // STEVILKA TIP DAT_VELJ DAT_IZDAJE TEL_ST EMAIL KONTROLOR DAT_KONTROLE
        // DEJAVNOST SHEMA_PROIZVODA ZP_IK NOSILEC_KMGMID NOSILEC_MATICNA CLAN_KMGMID
        // CLAN_MATICNA PRILOGA_STEVILKA PRILOGA_DAT_VELJ PRILOGA_DAT_VELJ
        ...
        for (ArrayList<String> row : excel) {
            try {
                ...
                Optional<ZascitenProizvod> zpExists = this.zascitenProizvodService
                        .getByIdShemaAndNaziv(shemaExists.get().getId(), zp);
                ...
                if (tip.equals("S")) {
                    ...
                    if (cert.getId() > 0 && (cert.getCertifikatPrilogaClan() != null
                            && cert.getCertifikatPrilogaClan().spliterator().getExactSizeIfKnown() > 0)) {
                    ...
                    for (ArrayList<String> clanRow : this.subjektService.getMembersFromExcel(stevilka, index, excel)) {
                        ...
                        try {
                            Subjekt clan = this.subjektService.getAndSaveBeforeIfNecessary(
                                    this.subjektService.getSubjektForImport(kmgmid_clan, maticna_clan));
                ...
                // This is only for a case, if we manage grouped certificates. Every certificate
                // is executed only ones.
                current = stevilka;
            } catch (Exception e) {
    ...
    public RefreshResponse refresh(Certifikat cert) throws HandledException {
        // HOW IT WORKS?
        //
        // Method return whole set of members and owner, if there were any changed made
        // in view data.

        RefreshResponse result = new RefreshResponse();

        boolean notMembers = (cert.getCertifikatPrilogaClan() == null
                || cert.getCertifikatPrilogaClan().spliterator().getExactSizeIfKnown() == 0);
        ...
        if (!notMembers) {
            ...
            Priloga priloga = cert.getCertifikatPrilogaClan().iterator().next().getPriloga(); // priloga mora biti ena
                                                                                              // in edina

            for (PrilogaClan prilogaClan : priloga.getPrilogaClan()) {
                Optional<Subjekt> newMember = this.subjektService
                        .getLastAndSaveBeforeIfNecessary(prilogaClan.getClan().getSubjId());
