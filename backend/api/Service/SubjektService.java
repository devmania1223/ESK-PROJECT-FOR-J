...
public class SubjektService extends BaseService {
    ...
    public Subjekt getAndSaveBeforeIfNecessary(Subjekt subjekt) {
        Optional<Subjekt> found = dbService.findFirstBySubjIdAndDatZsGreaterThanEqualOrderByIdDesc(subjekt.getSubjId(),
                subjekt.getDatZs());
    ...
    public ArrayList<Subjekt> importFromExcel(InputStream inputStream)
            throws HandledException, IOException, InvalidFormatException {
        // NOSILEC_KMGMID NOSILEC_MATICNA
        String errors = "";
        int columnCount = 2;
        if (!ExcelReader.isColumnsValid(inputStream, columnCount)) {
            errors = "Vpišite ime stolpca matična v prvi vrstici.";
        ...
        for (ArrayList<String> row : excel) {
            try {
                ...
                for (int i = 0; i < index - 2; i++) {
                    if (kmgmid != null && excel.get(i).get(0) != null && excel.get(i).get(0).equals(kmgmid)) {
                        error = "KMG-MID " + kmgmid + " je večkrat naveden."
                                + System.lineSeparator();
                        throw new HandledException(error);
                    } else if (maticna != null && excel.get(i).get(1) != null && excel.get(i).get(1).equals(maticna)) {
                        error = "Matična številka " + maticna + " je večkrat naveden."
                                + System.lineSeparator();
                        throw new HandledException(error);
                    } else if (i < result.size() && kmgmid != null && result.get(i).getKmgmid() != null
                            && result.get(i).getKmgmid().equals(kmgmid)) {
                        error = "KMG-MID " + kmgmid + " in MŠ " + result.get(i).getMaticna()
                                + " se nanašata na isti subjekt." + System.lineSeparator();
                        throw new HandledException(error);
                    } else if (i < result.size() && maticna != null && result.get(i).getMaticna() != null
                            && result.get(i).getMaticna().equals(maticna)) {
                        error = "KMG-MID " + result.get(i).getKmgmid() + " in MŠ " + maticna
                                + " se nanašata na isti subjekt." + System.lineSeparator();
                        throw new HandledException(error);
                    }
                }
    ...
    public ArrayList<ArrayList<String>> getMembersFromExcel(String stevilka, int row,
    ...
    public Subjekt getSubjekt(String kmgmid, String maticna) throws HandledException {
        ...
        if (maticna != null)
            subjekt = this.getFromViewByMaticna(maticna);
        else
            subjekt = this.getFromViewByKmgmid(kmgmid);

        if (!subjekt.isPresent()) {
            throw new HandledException("Subjekt ne obstaja. (KMGMID=" + Utils.getValueOrDefault(kmgmid) + ", matična="
                    + Utils.getValueOrDefault(maticna) + ")");
        }
        ...
        if (!res.getStatus().equals("S")) {
            if (kmgmid != null)
                throw new HandledException("KMG-MID " + Utils.getValueOrDefault(kmgmid) + " je neaktiven.");
            if (maticna != null)
                throw new HandledException("MATICNA " + Utils.getValueOrDefault(maticna) + " je neaktiven.");
        }
    ...
    public Subjekt getSubjektForImport(String kmgmid, String maticna) throws HandledException {
        ...
        if (maticna != null)
            subjekt = this.getFromViewAllByMaticna(maticna);
        else
            subjekt = this.getFromViewAllByKmgmid(kmgmid);
